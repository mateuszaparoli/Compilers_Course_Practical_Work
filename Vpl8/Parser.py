...
class Parser:
    ...
    def parse(self):
        ...
        """
        exp = self.expression()
        # Se ainda restar token não consumido, é erro de parse
    ...
    def expression(self):
        return self.fn_or_if_expr()
    
    def fn_or_if_expr(self):
    ...
    def unary_expr(self):
        ...
        if token and token.kind.name == 'NEG':
            ...
            exp = self.unary_expr()
            return Not(exp)
        elif token and token.kind.name == 'LET':
            self.eat()
    ...
    def application(self):
        left = self.primary()
        return self.application_rest(left)
    
    def application_rest(self, left):
        token = self.current_token()
        # Se o próximo token pode iniciar uma expressão primária, tratamos como aplicação
        if token and token.kind.name in ['LPR', 'NUM', 'TRU', 'FLS', 'VAR', 'FNX']:
            right = self.primary()
            app = App(left, right)
            return self.application_rest(app)
        return left
